...
$(function () {
    $('#datagrid_sbjc').datagrid({
        ...
        toolbar: [{
            ...
            text: '删除',
            handler: function () {
                var bsz = $('#datagrid_sbjc').datagrid('getChecked');
                if (bsz.length == 0) {
            ...
            handler: function () {
                ...
                $.ajax({
                    ...
                    success: function (r) {
                        ...
                        //设置疑似商标初审公告日
                        var startDate = r.obj.startDate;
                        if (startDate != "") {
                            date = new Date(parseInt(startDate));
                            var d2 = date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date.getDate() + '  ' + date.getHours() + ":" + date.getMinutes();
                            ;
                            $("#startDate").datebox('setValue', d2)//给初审公告日
                        }
                        //设置疑似商标异议截止日
                        var endDate = r.obj.endDate;
                        if (endDate != "") {
                            date = new Date(parseInt(endDate));
                            var d3 = date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date.getDate() + '  ' + date.getHours() + ":" + date.getMinutes();
                            $("#endDate").datebox('setValue', d3)//给疑似商标异议截止日
                        }
                    }
                });
...
function dialog_add_save() {
    if($('#img_add').val()=="" ||$('#img_add').val() ==undefined){
        $.messager.alert('信息','未上传疑似商标标样');
        return;
    }
    $.ajax({
        type: "POST",
...
//清空查询条件
function btn_clear() {
    $('#sbjc_sb_type').textbox('setValue', '');
    $('#sbjc_applyNumber').textbox('setValue', '');
    $('#sbjc_apply_date_start').datebox('setValue', '');
    $('#sbjc_apply_date_end').datebox('setValue', '');

    $('#datagrid_sbjc').datagrid('load', {
        sb_type: $('#sbjc_sort').val(),
        apply_number: $('#sbjc_applyNumber').val(),
        apply_date_start: $('#sbjc_apply_date_start').datebox('getValue'),
        apply_date_end: $('#sbjc_apply_date_end').datebox('getValue'),
