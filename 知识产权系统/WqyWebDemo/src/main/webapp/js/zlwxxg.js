...
$(function () {
    $('#datagrid_zlwxxg').datagrid({
        ...
        toolbar: [{
            ...
            text: '删除',
            handler: function () {
                var bsz = $('#datagrid_zlwxxg').datagrid('getChecked');
                if (bsz.length == 0) {
                ...
                $.messager.confirm('警告', '确认删除该行数据！', function (r) {
                    if (r) {
                        $.ajax({
                            ...
                            success: function (r) {
                                alert(r.msg);
                                $('#datagrid_zlwxxg').datagrid('reload');
                            }
                        });
                    }
                })
...
function showFile1(value, row, index) {
    if (row.invalid_files) {
        var btn = '<a class="look" onclick="dialog_showFile(\'' + row.invalid_files + '\')" href="javascript:void(0)">查看文件</a>';
        return btn;
    }
}
...
function btn_clear() {
    ...
    $('#zlwxxg_patentName').textbox('setValue','');
    $('#zlwxxg_technical_classification').textbox('setValue','');
    $('#zlwxxg_protected_areas').textbox('setValue','');
    $('#zlwxxg_apply_name').textbox('setValue','');

    $('#zlwxxg_apply_date_start').datebox('setValue','');
    $('#zlwxxg_apply_date_end').datebox('setValue','');
    $('#zlwxxg_accredit_date_start').datebox('setValue','');
    $('#zlwxxg_accredit_date_end').datebox('setValue','');

    $('#datagrid_zlwxxg').datagrid('load', { hid_value_address:$('#hid_value_address').text()});
}

//进行查询
function btn_search() {
    $('#datagrid_zlwxxg').datagrid('load', {
        patentName: $('#zlwxxg_patentName').textbox('getValue'),
        technical_classification: $('#zlwxxg_technical_classification').textbox('getValue'),
        protected_areas: $('#zlwxxg_protected_areas').textbox('getValue'),
        apply_name: $('#zlwxxg_apply_name').textbox('getValue'),

        apply_date_start: $('#zlwxxg_apply_date_start').datebox('getValue'),
        apply_date_end: $('#zlwxxg_apply_date_end').datebox('getValue'),
        accredit_date_start: $('#zlwxxg_accredit_date_start').datebox('getValue'),
        accredit_date_end: $('#zlwxxg_accredit_date_end').datebox('getValue'),
